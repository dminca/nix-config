{
  config,
  lib,
  ...
}:
let
  cfg = config.profiles.common.git;
in
{
  options.profiles.common.git = {
    enable = lib.mkEnableOption "shared Home Manager git profile";
  };

  config = lib.mkIf cfg.enable {
    home.file = {
      "${config.xdg.configHome}/git/git-commit-template.commit".source =
        ../../../hosts/common/dotfiles/git-commit-template.commit;
    };

    programs.git = {
      enable = true;
      settings = {
        delta = {
          navigate = true;
          side-by-side = true;
        };
        core = {
          editor = "nvim";
          logallrefupdates = true;
          sharedRepository = "0644";
          compression = "9";
        };
        commit = {
          template = "${config.xdg.configHome}/git/git-commit-template.commit";
        };
        color = {
          ui = true;
        };
        push = {
          default = "simple";
        };
        status = {
          submoduleSummary = true;
        };
        diff = {
          submodule = "log";
        };
        merge = {
          log = true;
          tool = "vimdiff";
        };
        difftool = {
          prompt = false;
        };
        mergetool = {
          prompt = false;
          keepBackup = false;
        };
        fetch = {
          recurseSubmodules = "on-demand";
          prune = true;
        };
        receive = {
          fsckObjects = true;
          denyDeletes = true;
          denyDeleteCurrent = true;
          denyCurrentBranch = true;
          denyNonFastForwards = true;
        };
        rebase = {
          stat = true;
        };
        filter."lfs" = {
          clean = "git-lfs clean -- %f";
          smudge = "git-lfs smudge -- %f";
          process = "git-lfs filter-process";
          required = true;
        };
      };
      ignores = [
        ".DS_Store"
        "*.pyc"
        ".idea"
        ".vscode"
        ".terraform.lock.hcl"
      ];
    };

    programs.delta = {
      enable = true;
      enableGitIntegration = true;
    };
  };
}
